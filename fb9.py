...
def nalm(yw, nalmparam, ig):
    ...
    yfnalm[0] = ifft(yf)*(0.6**0.5)
    ybnalm[0] = ifft(yb)*1j*(0.4**0.5)
...
def cavitystruct():
    ...
    # passive fiber
    betp = np.array([6.276e-2, 8.119e-5, 8.119e-5, -3.032e-10, -4.196e-13, -2.570e-16])
    betp = beta
    Aaf = np.pi * (9 *1e-6) ** 2 / 4 # square of fiber core
    n2 = 2.5e-20
    gamp1 = n2 * w0 / c / Aaf
    ...
    # filter
    lcenter = lmc + 0.5# filter center in wavelength
    fwidth = 1 # filter width
    # end filter
    """
...
def initdata(twidth, n1):
    ...
    y = 0.3*np.exp(-(t / 50) ** 2) # seed pulse
...
def initxlsfile():
    ...
    sheet['B1']= '-3dB durtn, ps'
    sheet['B1'].font = Font(bold=True)
    sheet['C1']= 'compr -3dB durtn, ps'
    sheet['C1'].font = Font(bold=True)
    sheet['D1']= '-3dB spctrm wdth, nm'
    sheet['D1'].font = Font(bold=True)
    sheet['E1']= 'TBP'
    sheet['E1'].font = Font(bold=True)
    sheet['F1']= 'compr TBP'
    sheet['F1'].font = Font(bold=True)
    sheet['G1']= '-60dB durtn, ps'
    sheet['G1'].font = Font(bold=True)
    sheet['H1']= 'compr -60dB durtn, ps'
    sheet['H1'].font = Font(bold=True)
    sheet['I1']= '-60dB spctrm wdth, nm'
    sheet['I1'].font = Font(bold=True)
    sheet['J1']= 'peak intst, W'
    sheet['J1'].font = Font(bold=True)
    sheet['K1']= 'enrj, nJ'
    sheet['K1'].font = Font(bold=True)
    sheet['L1']= 'nalm passiveD L, m'
...
def report(ywtlmd, Pp, Ip, Ep):
    ...
    asd = ((np.abs(flt) ** 2) / np.max(np.abs(flt) ** 2))
    flt = 10 * np.log10(asd)
    flt = fftshift(flt)
    # end filter
    ...
    nalmff1 = np.angle(yfnalm[0])
    nalmfb1 = np.angle(ybnalm[0])
...
def savetoxls(ii, yout):
    ...
    sheet.cell(row=ii2, column=1).value = ii2
    sheet.cell(row=ii2, column=2).value = TBPt
    sheet.cell(row=ii2, column=3).value = TBPtcomp
    sheet.cell(row=ii2, column=4).value = TBPlmd
    sheet.cell(row=ii2, column=5).value = TBPdata
    sheet.cell(row=ii2, column=6).value = TBPdatacomp
    sheet.cell(row=ii2, column=7).value = TBPt60
    sheet.cell(row=ii2, column=8).value = TBPtcomp60
    sheet.cell(row=ii2, column=9).value = TBPlmd60
    sheet.cell(row=ii2, column=10).value = Ip
    sheet.cell(row=ii2, column=11).value = Ep/1e-9
    sheet.cell(row=ii2, column=12).value = CavityParamToSave
...
multiple2savedata = 10 # cavity round trip multiple to save pulse data to file
...
frames0 = 100 # number of cavity runs
